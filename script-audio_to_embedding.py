...
target_file = 'reading'
audio_path = './field_study/field_data/P2/segments/%s.wav' % target_file # path of the audio clip
label_path = './field_study/field_data/P2/labels/%s.csv' % target_file # path of the csv
save_csv_dir = './field_study/field_data/P2/spectrogram_1s/%s/' % target_file # dir to save segments of spectrogram (can be used for embedding extraction)
save_context_dir = './field_study/field_data/P2/contextual_1s/%s/' % target_file # dir to save contextual MFCCs
...
save_embedding_path = './field_study/field_data/P2/embedding_1s/'
save_mfcc_path = './field_study/field_data/P2/mfcc_1s/'
...
check_dirs.check_dir(save_context_dir)
for seg_idx in range(n):
    label = labels[seg_idx][0]
    if extract_spec:
        seg = inpt[:, seg_idx * (srate // hop_length):(seg_idx+1) * (srate // hop_length)]
    # compute mean and contextual (with time axis) acoustic features
    mean = np.empty((1,0))
    contextual = np.empty((0, srate // hop_length))
    for feat_type in [mfcc_feat, mfcc_delta_1, mfcc_delta_2, rms, rms_delta_1, zcr, zcr_delta_1, f0, f0_delta_1]:
        texture = feat_type[:, seg_idx * (srate // hop_length):(seg_idx+1) * (srate // hop_length)]
        mean = np.append(mean, np.mean(texture, axis=1))
        contextual = np.vstack((contextual, texture))
    mean = np.append(mean, label)
    acoustic_mean = np.hstack((acoustic_mean, mean.reshape((num_mfcc*3+6+1, 1))))
    
    # save mel-spectrogram for each 1-second segment
    ...
    if extract_spec:    
        ...
        csvfile.close()
    # save contextual MFCC and acoustic features for each 1-second segment
    # naming format: ./label_segx.csv, x is the segment idx
    with open(save_context_dir + str(label) + '_seg' + str(seg_idx) + ".csv", 'w', newline='') as csvfile:   # no gap in lines
            spamwriter = csv.writer(csvfile, delimiter=',')
            spamwriter.writerows(contextual)
    csvfile.close()
